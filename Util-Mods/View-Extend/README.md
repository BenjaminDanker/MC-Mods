...
- `PlayerViewPlanner` / `PlayerViewState`: lazy radial/strip demand, current-distance priority, retries, unloads, and session/request identity.
...
`[ViewExtend View]` reports each player's requested/effective/server distances, sent/pending/retry counts, bootstrap progress, remaining batch quota, and outstanding batches. The aggregate metrics also report lazy descriptor count, queued/read/completed/ready stage counts, nearest/farthest admitted/active/read/ready/sent distances, corrected/actual delivery inversions, stale skips, superseded subscribers, and unstarted orphan reads skipped. `preparedDropped` still includes unfinished chunks for comparison with old logs; use the reason counts to distinguish them from errors. `mainElapsedMsps` and `workerElapsedMsps` replace the misleading CPU labels; ordinary failed preparations now contribute to worker timing. Timed-out work still running on a worker is not included in that timing. Network figures remain pre-compression estimates.

`[ViewExtend Memory]` reports current and peak raw-NBT ownership, worker queue/active counts, copy overlap, READY/cache/unique prepared-byte estimates, quota-blocked READY bytes, waiting time, orphaned work, and primitive player-state estimates. Raw NBT is measured once with `CompoundTag.sizeInBytes()` when a completed disk read enters the worker queue. Prepared sizes use the packet retained-byte estimator. `sentBytes~` and `schedulerBytes~` model FastUtil's 0.75-load-factor primitive arrays plus conservative object/descriptor overhead; they are not JVM heap measurements. `[ViewExtend Cache]` reports interval admissions, tier placement, hits, promotions, expiration, limit eviction, and unused expiry.
...
Observers receive byte and elapsed-time deficit credits. Candidate work rotates one accepted request at a time, prepared deliveries get a fair pass, and unused shares can then be borrowed. Admission, queued reads, and READY delivery use the same tier/current-Chebyshev-distance/age ordering; READY priority is rebuilt from the player's current center for each bounded delivery pass. Global packet, read, queue, cache and client-ACK limits still apply. Existing READY work is offered network quota before new read dispatch, while candidate admission can keep the bounded lookahead warm without consuming chunk-batch quota.
